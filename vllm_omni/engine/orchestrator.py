                ...
                for replica_id in range(pool.num_replicas):
                    ...
                    if pool.stage_type == "diffusion":
                        ...
                        try:
                            ...
                            processed_request_ids = {output.request_id for output in raw_output}
    ...
    async def _handle_processed_outputs(self, stage_id: int, replica_id: int, outputs: list[Any]) -> None:
        ...
        for output in outputs:
            ...
            if output.finished:
                ...
                stage_latency_time_ms = max(0.0, (stage_end_ts - submit_ts) * 1000.0)
                stage_metrics.output_processor_time_ms = output_processor_time_ms
                stage_metrics.stage_submit_ts = submit_ts
                stage_metrics.stage_end_ts = stage_end_ts
                stage_metrics.stage_latency_time_ms = stage_latency_time_ms
                stage_metrics.stage_gen_time_ms = max(0.0, stage_latency_time_ms - output_processor_time_ms)
                stage_metrics.request_dispatch_wait_time_ms = req_state.request_dispatch_wait_time_ms
    ...
    async def _forward_to_next_stage(
        ...
        *,
        ...
        if next_pool.stage_type == "diffusion":
            ...
            req_state.stage_submit_ts[next_logical] = next_submit_ts
            if stage_metrics is not None:
                stage_metrics.handoff_to_stage_id = next_logical
