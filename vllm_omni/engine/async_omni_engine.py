...
def _weak_shutdown_async_omni_engine(
    ...
    request_queue: janus.Queue[dict[str, Any]],
    output_queue: janus.Queue[dict[str, Any]],
    rpc_output_queue: janus.Queue[dict[str, Any]],
) -> None:
...
class AsyncOmniEngine:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Janus queues are constructed here (not deferred to the orchestrator
        # thread) so the master server's ROUTER thread always sees a non-None
        # ``self.request_queue`` when on_register fires. ``async_q`` lazily
        # binds to whatever event loop first awaits on it, which is the
        # orchestrator loop — so cross-thread use stays correct.
        self.request_queue: janus.Queue[dict[str, Any]] = janus.Queue()
        self.output_queue: janus.Queue[dict[str, Any]] = janus.Queue()
        self.rpc_output_queue: janus.Queue[dict[str, Any]] = janus.Queue()
    ...
    def _dispatch_master_register(self, stage_id: int, replica_id: int, alloc: Any) -> None:
        ...
        is created in :meth:`__init__` before the master server starts,
        so it is always non-None here; ``janus.Queue.sync_q`` is
        thread-safe by construction.
        """
        msg = {
            "type": "register_remote_replica",
